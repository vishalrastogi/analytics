...
#Vectors----
x=1:10    #create seq of nos. for 1 to 10
x
x1 <- 1:20
x1

(x1=1:30)     #assign and print altogether
(x2=c(1,2,13,4,5))
class(x2)
class(x)
(x3=letters[1:10])
...
x6[3]
x6[c(2,4)]
x6[-1]    # all but 1st element
x6[-c(1:10,15:20)]
x6[c(2.56,13.52)]
x6[-(length(x6)-1)]
#can not mix positive and negative
(x7 = c(x6,x2))
...
plot(density(x1))
abline(v=mean(x1),h=0.04)    #plotting a line in graph
...
addmargins(m1,margin = 2,sum) #rowwise
